# NIST EDA 1.3.3.13.DOE Standard Deviation Plot####
# Date: 20260406
...
# 説明 ####
# NIST Statistic Handbook 1.3.3.13.DOE 標準偏差プロット
# 1.3.3.12 のDOE平均プロットのmeanをsdに、ラベルを平均を標準偏差に置き換えた
# デフォルト環境の保存と調整 ####
...
# 標準偏差を計算####
sd_data <- aggregate(Y ~ Level + Factor, data = long_data, sd)
grand_sd <- sd(long_data$Y)

# 枠組み描画####
plot(NULL, xlim=c(0.5, 7.5), ylim=range(sd_data$Y), 
     xaxt="n", xlab="因子", ylab="標準偏差", main="DOE 標準偏差プロット")
#rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "lightyellow") # 背景色
abline(h = grand_sd, lty = 2, col = "red") # 総平均

# 因子ごとに線と点を描画####
factors <- unique(sd_data$Factor)
for(i in 1:length(factors)){
  sub <- sd_data[sd_data$Factor == factors[i], ]
