[原文 (Original English)](https://www.itl.nist.gov/div898/handbook/eda/section4/eda42a1.htm)  
閲覧(UTC)：2026-04-24 16:54:34  
[⬅️](eda42a.md)[➡️](eda42a2.md)  
...
#### 作成
本事例研究のデータは、1996年にNISTセラミックス部門の Said Jahanmir によって、セラミックスの強度最適化を目的としたNISTと産業界のセラミックス共同研究プロジェクトの一環として収集されたものである。

本データ集合を研究する動機は、実験計画に基づく多因子分析を例示することにある。

本事例研究では、NIST統計工学部門の Lisa Gill および James Filliben によって実施された全調査データの一部のみを利用する。

応答変数は、セラミックス材料（結合型硝酸塩）の強度の測定値である。
完全なデータ集合には、以下の変数が含まれている。

1. 因子1 = 観測ID、すなわち実験回番号（1～960） 
2. 因子2 = 試験所（1～8） 
3. 因子3 = 試験所内の試験片ID（1～30） 
4. 因子4 = 試験番号（1～4） 
5. 応答変数 = セラミック材料の強度  
6. 因子 5 = テーブル速度（2 レベル：0.025 および 0.125） 
7. 因子 6 = 送り下り速度（2 レベル：0.050 および 0.125） 
8. 因子 7 = ホイールの砥粒サイズ（2 レベル：150 および 80） 
9. 因子 8 = 方向（2 レベル：縦方向および横方向） 
10. 因子 9 = 処理（1～16） 
11. 因子 10 = 試験所内での 15 セット（2 レベル：1 および 2） 
12. 因子 11 = 反復（2 レベル：1 および 2） 
13. 因子12 = バーのバッチ（1および2） 
...
この事例研究では、データの半分のみを使用している。
具体的には、方向が縦方向のデータを使用している。
したがって、主因子は3つだけとなる

さらに、我々は以下の混在要因にも注目している。
...
1. 4つの主因子のうち、どれがセラミック材料の強度に対して最も強い影響を与えるかを特定する  
2. それらの影響の大きさを推定する  
3. 主因子の最適設定を決定する  
4. 局外因子（試験所因子およびロット因子）がセラミック強度に影響を与えるかどうかを特定する  

本事例研究は実験計画の一例である。
[工程改善](../../pri/pri.md)の章には、実験計画の構築と解析に関する詳細な解説を収録している。
本事例研究は、EDA手法（図表現技法の活用を重視する）が実験計画の解析にどのように活用できるかを示すことで、同章の内容を補完することを目的としている。 

#### ソフトウェア
本事例研究で使用された分析は、[Dataplotコード](https://www.itl.nist.gov/div898/handbook/eda/section4/eda42a3.dp)と[Rコード](https://www.itl.nist.gov/div898/handbook/eda/section4/eda42a3.r)の両方を用いて実施することができる。
読者は、データを[テキストファイルとして](https://www.itl.nist.gov/div898/handbook/datasets/JAHANMI2.DAT)ダウンロードできる。

#### データ
この事例研究で使用したデータは以下の通り。 
