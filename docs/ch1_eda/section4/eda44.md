...
1 や 2 は有用な指標であるが、描画文字の選択はユーザーの裁量に委ねられている。
...
これらの内部母数は信頼区間を計算するのに有用であり、出力（PRINT BMEAN）したり、他のユーザー定義母数と同様に使用したりする（例：LET UCL = B95）。 
...
5. ユーザー指定の目標値における基準線
6. ユーザーが指定した上限仕様限界における基準線
7. ユーザーが指定した下限仕様限界における基準線 
...
上記の手法は、非線形最小二乗法による近似機能と、確率密度関数を定義する機能（組み込みまたはユーザー定義）を備えたあらゆる統計ソフトウェア・パッケージで
利用可能である。 
...
[1]: 原点では二項分布ではなくベータ分布のリンクになっています。(2026-04-30)
[2]: リンクURLセクション指定の間違い。本来は、"/eda/section3/xxx"が "/eda/section4/xxx" になっている。 (2026-04-30)
[3]: 原典のリンク先は、Poisson分布ではなく、二項分布になっています(2026-05-01)
